...
description: >
  진입 채널(BFF)이 여럿인 구조에서 새 로직을 추가할 때, 요청이 들어온 채널에 바로 넣기 쉽다.
  하지만 그 전에 "이 정책이 이 채널에만 해당하는가, 아니면 모든 채널이 지켜야 할 공통 논리인가"를
  먼저 물어야 한다. 채널별 BFF에 둘지 공통 도메인 서비스에 응집할지 판단한 흐름을 정리하고자 한다.
sitemap: false
