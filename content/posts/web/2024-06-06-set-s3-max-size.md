...
date: '2024-06-06'
category: web
description: 졸업 작품팀에서 나는 백엔드 파트 및 인프라 구축을 담당하게 되었다. S3의 최대 파일(오브젝트) 크기를 지정하기 위한 방법을 찾기 위한 트러블 슈팅 과정을 기술하고자 한다.
---
...
&nbsp; `Content Header 추가` 방식보다 비교적 설정이 복잡하다. 또한 마찬가지로 클라이언트 측 검증이기 때문에 클라이언트 조작에서 아주 자유롭지는 못하다. 마지막으로 정책 문서의 크기가 20KB로 제한되어 있기 때문에, 매우 복잡한 정책을 작성할 때 제한이 있을 수 있다.<br>
&nbsp; 추가적으로 발견한 점은 `POST Policy`이기 때문에 HTTP Method 역시 'PUT'에서 'POST'로 변경해주어야 한다는 점이다. 기존에 Presigned URL을 통한 정적 파일을 저장하는 API가 존재할 경우 이에 대한 혼동이 있을 수 있기 때문에 유의해야 할 것으로 보인다.
