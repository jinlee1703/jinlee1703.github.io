...
date: '2025-08-17'
category: web
description: 사이드 프로젝트를 진행하며 백오피스를 개발하게 되었고, 그 과정에서 관리자 로그인 페이지 구현을 맡게 되었다. 인증 방식으로 JWT 기반 접근 방식을 선택하였고, Access Token의 탈취를 방지하기 위해 짧은 TTL을 설정하였다. 그러나 구현 도중 "만약 Refresh Token이 탈취된다면 어떻게 대응해야 할까?"라는 의문이 생겼고, 이를 해결할 수 있는 방안을 탐색하게 되었다.
---
